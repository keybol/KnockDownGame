	...
	public sealed class BzRagdoll : MonoBehaviour, IBzRagdoll
	{
		...
		public void AddExtraMove(Vector3 move)
		{
			if (IsRagdolled)
			{
				...
				airMove = new Vector3(move.x, 2f, move.z);
				_hipsTransformRigid.AddForce(airMove, ForceMode.VelocityChange);
				_hipsTransformRigid.angularVelocity = Vector3.zero;
				foreach (var rigid in _rigids)
				{
					rigid.RigidBody.velocity = airMove;
					rigid.RigidBody.angularVelocity = Vector3.zero;
				}
			}
		}
		#endregion
		...
		IEnumerator FindHips()
		{
			yield return new WaitForSeconds(0.1f);
			_hipsTransform = kPlayer._hipsTransform;
			_hipsTransformRigid = kPlayer._hipsTransformRigid;
		}
		...
		public void RagdollIn()
		{
			...
			kPlayer.kcc.RestrictMovement = true;
			_anim.enabled = false;      // disable animation
			_state = RagdollState.Ragdolled;
			//ApplyVelocity(kPlayer.kcc.Motor.Velocity);
		}
		...
		/// </summary>
		public void ApplyVelocity(Vector3 predieVelocity)
		{
			foreach (var rigid in _rigids)
		...
		private void ActivateRagdollParts(bool activate)
		{
			...
			foreach (var rigid in _rigids)
			{
				...
				if (partColider == null)
				{
					...
					Transform transform = rigid.RigidBody.transform.FindChild(childName);
